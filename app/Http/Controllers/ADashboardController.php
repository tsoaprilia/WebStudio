...
class ADashboardController extends Controller
{
    public function dashboard()
    {
        ...
        $graphData = $this->productPurchaseGraph();

        return view('ADashboard', compact('userCount', 'orderCount', 'uiUxProductCount', 'feedsProductCount', 'totalEarnings', 'totalProductsCount', 'mostPurchasedProduct', 'mostActiveUser', 'graphData'));
    }

    public function productPurchaseGraph()
{
    $products = Product::withCount('transactions')->orderByDesc('transactions_count')->limit(5)->get();

    // Penjualan product tertingga
    $productName = $products->pluck('name_product');
    $totalSold = $products->pluck('transactions_count');

    return compact('productName', 'totalSold');
}
